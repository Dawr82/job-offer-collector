...
MONGO_DB_JOB_OFFER_DATABASE = os.getenv(
    "MONGO_DB_JOB_OFFER_DATABASE", default="scraper"
)

redis_client = redis.Redis(
    host=REDIS_DB_HOSTNAME, port=REDIS_DB_PORT, db=REDIS_DB_DATABASE
)
        ...
        if request.args:
            ...
            for key in get_keys:
                if key not in ("count", "sort"):
                    data = self.filter(
                        key, request.args.get(key).lower().capitalize(), data
                    )
                    break
...
def not_found_error_handler(error):
    return {"statusCode": 404, "description": "Not Found"}
