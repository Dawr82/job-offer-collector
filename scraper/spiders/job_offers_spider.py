    ...
    def set_logging(cls):
        try:
            formatter = logging.Formatter(
                "[%(asctime)s] %(message)s", settings.CRAWLER_LOG_DATETIME_FORMAT
            )
            handler = logging.FileHandler(
                settings.CRAWLER_LOG_FILE, settings.CRAWLER_LOG_MODE
            )
...
class NFJJobOfferSpider(BaseJobOfferSpider):
    ...
    def parse_content(self, offer_content):
        ...
        try:
            ...
            loader.add_css(
                "seniority", "div.row.mb-3 [id=posting-seniority] span::text"
            )
            loader.add_css("company", "div.row.mb-3 [id=postingCompanyUrl]::text")
            loader.add_css("salary", "div.row.mb-3 div.salary h4::text")
            loader.add_css(
                "required",
                "div [id=posting-requirements] [class=d-block] h3.mb-0 button::text",
            )
            loader.add_css(
                "required",
                "div [id=posting-requirements] [class=d-block] h3.mb-0 a::text",
            )
            loader.add_css(
                "optional",
                "div [id=posting-requirements] [id=posting-nice-to-have] h3.mb-0 button::text",
            )
            loader.add_css(
                "optional",
                "div [id=posting-requirements] [id=posting-nice-to-have] h3.mb-0 a::text",
            )
            loader.add_css("remote", "div.row.mb-3 [maticon=home]")
            loader.add_css(
                "locations", "div.row.mb-3 [popoverplacement=bottom] span::text"
            )
            item = loader.load_item()
            item["offer_id"] = hashlib.md5(
                json.dumps(dict(item), sort_keys=True).encode()
            ).hexdigest()
        except Exception as error:
            self.logger.error(
                f"{type(error).__name__}: {error} URL -> {offer_content.url}"
            )
        return item
